class CompraDetalleManager {
    ...
    async actualizarTablaCompras() {
        try {
            console.log('Actualizando tabla de compras desde CompraDetalleManager...');
            ...
            // Fallback: usar CRMApplication si está disponible
            if (window.crmApp && window.crmApp.forceRefreshComprasTable) {
...
async function verDetallesCompra(compraId, compraNum) {
    try {
        ...
        if (!window.compraDetalleManager) {
            console.log('Inicializando CompraDetalleManager...');
            window.compraDetalleManager = new CompraDetalleManager();
        }
        
        // Usar el método abrir (no abrirModal)
        ...
        window.notificationService.error('Error al abrir los detalles de la compra');
    }
}
