class CompraDetalleManager {
                ...
                <div class="modal-content" style="max-width: 900px;">
                    ...
                    <div class="modal-body">
                        <div class="section-header">
                            ...
                            <button onclick="compraDetalleManager.agregarItem()" class="btn btn-primary">
    ...
    async actualizarTablaCompras() {
        try {
            console.log('🔄 Actualizando tablas después de cambio en detalles...');
            
            // 1. Actualizar tabla de compras
            if (window.smartCache) {
                window.smartCache.invalidate('collection_TB_COMPRAS');
                console.log('Cache TB_COMPRAS invalidado');
            }
            ...
            // 2. NUEVO: Actualizar tabla de productos
        ...
        try {
            ...
            if (window.paginationManagers && window.paginationManagers['productos']) {
                ...
                if (isOnProductosTab) {
                    ...
                    console.log('✅ Tabla de productos actualizada desde CompraDetalleManager');
                }
            }
...
async function verDetallesCompra(compraId, compraNum) {
    try {
        console.log(`verDetallesCompra called with: ID=${compraId}, NUM=${compraNum}`);
        
        // Asegurar que el manager existe
        if (!window.compraDetalleManager) {
            console.log('Creating new CompraDetalleManager instance...');
            window.compraDetalleManager = new CompraDetalleManager();
        }
        
        // Verificar que los parámetros son válidos
        if (!compraId || !compraNum) {
            throw new Error('ID de compra y número son requeridos');
        }
        
        await window.compraDetalleManager.abrir(compraId, compraNum);
        console.log('Compra details modal opened successfully');
        
    } catch (error) {
        console.error('Error opening compra details:', error);
        window.notificationService.error('Error al abrir detalles de compra: ' + error.message);
    }
}

// Inicializar manager global de forma segura
document.addEventListener('DOMContentLoaded', () => {
    try {
        if (!window.compraDetalleManager) {
            window.compraDetalleManager = new CompraDetalleManager();
            console.log('CompraDetalleManager initialized on DOM ready');
        }
    } catch (error) {
        console.error('Error initializing CompraDetalleManager on DOM ready:', error);
    }
});
...
document.addEventListener('DOMContentLoaded', () => {
    if (!window.compraDetalleManager) {
        console.log('Auto-inicializando CompraDetalleManager en DOMContentLoaded...');
        window.compraDetalleManager = new CompraDetalleManager();
    }
});