class ModalManager {
    ...
    async setupCompraForm(editingId) {
        try {
            ...
            // Cargar dropdowns - USAR FUNCIONES ESPECÍFICAS PARA COMPRAS
            await Promise.all([
                this.loadProviderDropdown(),
                this.loadCompraPaymentTypeDropdown(),     // <- CAMBIADO DE loadPaymentTypeDropdown
                this.loadCompraPaymentTypeDropdown2()     // <- CAMBIADO DE loadPaymentTypeDropdown2
            ]);
            ...
            // Configurar eventos para tipo de pago (si es necesario en el futuro)
            const tiposPago = document.getElementById('field-COM_TIPO_PAGO');
            if (tiposPago) {
                tiposPago.addEventListener('change', (e) => {
                    console.log('Payment type changed:', e.target.value);
                    // Aquí se puede agregar lógica para manejar cambios en tipo de pago
                });
            }
...
// 3. AGREGAR función para cargar proveedores:
    ...
    async loadCompraEditData(data) {
        try {
            ...
            // Cargar dropdowns con valores seleccionados - USAR FUNCIONES CORRECTAS
