class ModalManager {
    ...
    async setupCompraForm(editingId) {
        try {
            ...
            // CORREGIDO: Usar funciones específicas para compras
            await Promise.all([
                this.loadProviderDropdown(),
                this.loadCompraPaymentTypeDropdown(),     // <- CAMBIADO
                this.loadCompraPaymentTypeDropdown2()     // <- CAMBIADO  
            ]);
            ...
            console.log('Compra form setup completed successfully');
...
// 3. AGREGAR función para cargar proveedores:
    ...
    async loadCompraEditData(data) {
        try {
            console.log('Loading compra edit data:', data);
            
            // Cargar datos básicos - ASEGURAR QUE TODOS LOS CAMPOS SE CARGUEN
            Object.keys(data).forEach(key => {
                const field = document.getElementById(`field-${key}`);
                // Excluir solo campos que se manejan con dropdowns especiales
                if (field && !['COM_PROVEEDOR', 'COM_TIPO_PAGO', 'COM_TIPO_PAGO2'].includes(key)) {
                    field.value = data[key] || '';
                    console.log(`Loaded field ${key}:`, data[key]);
                }
            });

            // CORREGIDO: Cargar dropdowns con funciones específicas de compra
            await Promise.all([
                this.loadProviderDropdown(data.COM_PROVEEDOR),
                this.loadCompraPaymentTypeDropdown(data.COM_TIPO_PAGO),      // <- CAMBIADO
                this.loadCompraPaymentTypeDropdown2(data.COM_TIPO_PAGO2)     // <- CAMBIADO
            ]);
            
            // Verificar que los campos específicos se cargaron correctamente
            console.log('=== COMPRA EDIT VERIFICATION ===');
            const fieldsToCheck = [
                'COM_ESTADO_SERVICIO',
                'COM_FECHA_PAGO_SALDO', 
                'COM_FECHA_RECEPCION',
                'COM_TIPO_PAGO',
                'COM_TIPO_PAGO2'
            ];
            ...
            fieldsToCheck.forEach(fieldName => {
                ...
                console.log(`${fieldName}: element exists = ${!!element}, data value = "${dataValue}", element value = "${element?.value}"`);
            });
            ...
            console.log('✅ Compra edit data loaded successfully');
    ...
    async loadCompraPaymentTypeDropdown(selectedPaymentId) {
        ...
        if (!paymentSelect) {
            console.warn('field-COM_TIPO_PAGO select not found');
            return;
        }

        try {
            console.log('Loading payment types for compra...');
            const paymentTypes = await window.dataService.getAll('tipos_pago');
            ...
            console.log('Compra payment types loaded successfully');
    ...
    async loadCompraPaymentTypeDropdown2(selectedPaymentId) {
        ...
        if (!paymentSelect) {
            console.warn('field-COM_TIPO_PAGO2 select not found');
            return;
        }

        try {
            console.log('Loading payment types 2 for compra...');
            const paymentTypes = await window.dataService.getAll('tipos_pago');
            ...
            console.log('Compra payment types 2 loaded successfully');
    ...
    // Función para cargar tipos de pago específica para compras (campo secundario)
    async loadCompraPaymentTypeDropdown2(selectedPaymentId) {
        const paymentSelect = document.getElementById('field-COM_TIPO_PAGO2');
        if (!paymentSelect) {
            console.warn('field-COM_TIPO_PAGO2 select not found');
