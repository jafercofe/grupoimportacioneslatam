class ModalManager {
    ...
    async handleSubmit() {
        try {
            ...
            if (this.editingItem) {
                // Verificar si es una compra que se está marcando como terminada
                const isCompraBeingFinished = this.currentType === 'compra' && 
                                            formData.COM_ESTADO_SERVICIO === 'Terminado';
                
                if (isCompraBeingFinished) {
                    // Mostrar loading específico para procesamiento de inventario
                    const loadingToast = window.notificationService.info('Actualizando compra y procesando inventario...', 0);
                    
                    try {
                        const inventoryResult = await window.dataService.update(collectionName, this.editingItem, formData);
                        
                        // Limpiar toast de loading
                        window.notificationService.remove(loadingToast);
                        
                        if (inventoryResult && inventoryResult.success) {
                            window.notificationService.success('✅ Compra actualizada e inventario procesado exitosamente');
                            
                            // Refrescar múltiples tablas
                            await this.refreshMultipleTables(['compras', 'productos']);
                        } else {
                            window.notificationService.success('Compra actualizada (inventario ya procesado)');
                        ...
                        window.notificationService.error('Error al actualizar compra y procesar inventario');
                        return;
                    }
                } else {
                ...
                if (this.currentType === 'pedido') {
                    ...
                    // Esperar un momento para que se cierre el modal actual
                    setTimeout(async () => {
                        if (window.verDetallesPedido) {
                            await window.verDetallesPedido(newId, pedidoNum);
                        }
                    }, 300);
                    return; // Salir sin hacer refresh todavía
                }
    ...
    async refreshMultipleTables(tableNames) {
        try {
            console.log('Refrescando múltiples tablas:', tableNames);
            ...
            console.log('Todas las tablas han sido refrescadas');
            
        } catch (error) {
            console.error('Error refrescando múltiples tablas:', error);
            // Fallback a refresh normal
            await this.refreshCurrentTable();
        }
    }
