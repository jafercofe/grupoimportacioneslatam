class ModalManager {
    ...
    async handleSubmit() {
        try {
            ...
            let inventarioActualizado = false; // NUEVO: Flag para rastrear cambios de inventario
            
            if (this.editingItem) {
                // Detectar cambio de estado en compras
                if (this.currentType === 'compra') {
                    // Obtener estado anterior
                    const compraActual = await window.dataService.getById(collectionName, this.editingItem);
                    const estadoAnterior = compraActual?.COM_ESTADO_SERVICIO;
                    const estadoNuevo = formData.COM_ESTADO_SERVICIO;
                    
                    console.log('🔄 Verificando cambio de estado de compra:', {
                        compraId: this.editingItem,
                        estadoAnterior,
                        estadoNuevo
                    });
                    ...
                    // Si cambió el estado, actualizar inventario
                    if (estadoAnterior !== estadoNuevo) {
                        console.log('🔄 Estado cambió, actualizando inventario...');
                        ...
                        try {
                            ...
                            console.log('✅ Inventario actualizado por cambio de estado:', resultado);
                            inventarioActualizado = true; // NUEVO: Marcar que se actualizó inventario
                            
                            // Mostrar notificación específica
                            if (estadoAnterior !== 'Terminado' && estadoNuevo === 'Terminado') {
                                window.notificationService.success(`✅ Compra terminada - Inventario actualizado (+${resultado.detallesProcesados} productos)`);
                            } else if (estadoAnterior === 'Terminado' && estadoNuevo !== 'Terminado') {
                                window.notificationService.info(`⏸️ Compra cancelada - Inventario revertido (-${resultado.detallesProcesados} productos)`);
                            }
                            
                        } catch (error) {
                            console.error('❌ Error actualizando inventario por cambio de estado:', error);
                            window.notificationService.error('Error al actualizar inventario: ' + error.message);
                        }
                    }
                    
                    // Recalcular total final si es necesario
                    await window.dataService.recalculateCompraFinalTotal(this.editingItem);
                    
                } else {
                    // Para otros tipos (no compras)
                    await window.dataService.update(collectionName, this.editingItem, formData);
            ...
            // NUEVO: Actualizar tabla de productos si hubo cambios de inventario
            if (inventarioActualizado) {
                console.log('📦 Actualizando tabla de productos por cambio de inventario...');
                await this.refreshProductosTable();
            }
    ...
    async setupCompraForm(editingId) {
        try {
            console.log('🛒 setupCompraForm ejecutándose', {
                editingId,
                modalClasses: Array.from(this.modal.classList),
                modalDataType: this.modal.getAttribute('data-type')
            });
            ...
            // Configurar campos según el modo (agregar/editar)
            this.configureCompraFormFields(editingId);
            
            // Agregar listener para cambios de estado SOLO si estamos editando
            if (editingId) {
                const estadoField = document.getElementById('field-COM_ESTADO_SERVICIO');
                if (estadoField) {
                    try {
                        // Obtener estado actual
                        const estadoActual = await this.getCurrentCompraEstado(editingId);
                        console.log('📋 Estado actual de la compra:', estadoActual);
                        
                        estadoField.addEventListener('change', (e) => {
                            const nuevoEstado = e.target.value;
                            console.log('🔄 Estado cambiará:', estadoActual, '→', nuevoEstado);
                            
                            // Mostrar advertencia visual
                            if (estadoActual !== nuevoEstado) {
                                if (estadoActual !== 'Terminado' && nuevoEstado === 'Terminado') {
                                    this.showEstadoWarning('✅ Al cambiar a "Terminado" se AUMENTARÁ el inventario de todos los productos de esta compra.');
                                } else if (estadoActual === 'Terminado' && nuevoEstado !== 'Terminado') {
                                    this.showEstadoWarning('⚠️ Al cambiar de "Terminado" se REDUCIRÁ el inventario de todos los productos de esta compra.');
                                } else {
                                    this.hideEstadoWarning();
                                }
                            } else {
