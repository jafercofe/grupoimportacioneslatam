class PaginationManager {
    ...
    renderPage(data) {
        ...
        if (this.collection === 'tb_pedido') {
            ...
            this.loadPaymentTypes();
            this.loadPaymentTypes2();
        }  else if (this.collection === 'tipos_trabajador') {  // ✅ NUEVO
            this.loadTiposTrabajadorPermissions();
        }
    ...
    createTableRow(item) {
            ...
            return `
                <tr>
                    ...
                    <td class="observaciones-cell">
                        <div class="observaciones-preview" title="${item.PED_OBS || 'Sin observaciones'}">
                            ${item.PED_OBS ? (item.PED_OBS.length > 50 ? item.PED_OBS.substring(0, 50) + '...' : item.PED_OBS) : '-'}
                        </div>
                    </td>
        ...
        else if (this.collection === 'tipos_trabajador') {
            return `
                <tr data-tipo-trabajador-id="${item.id}">
                    <td>${item.descripcion || ''}</td>
                    <td>
                        <div class="permissions-loading" data-tipo-id="${item.id}">
                            <div class="loading" style="width: 16px; height: 16px; display: inline-block; margin-right: 8px;"></div>
                            Cargando permisos...
                        </div>
                    </td>
                    <td>
                        <div class="flex gap-2">
                            <button onclick="editItem('tipo-trabajador', '${item.id}')" class="btn btn-warning btn-sm">
                                <i data-lucide="edit"></i>
                            </button>
                            <button onclick="deleteItem('tipo-trabajador', '${item.id}')" class="btn btn-danger btn-sm">
                                <i data-lucide="trash-2"></i>
                            </button>
                        </div>
                    </td>
    ...
    getItemType() {
        ...
        'tipos_trabajador': 'tipo-trabajador',
        'entregas': 'entrega',
        'empleados': 'empleado',
        'clientes': 'cliente'
    };
    ...
    async loadTiposTrabajadorPermissions() {
        try {
            console.log('🔐 Cargando permisos para tipos de trabajador...');
            
            const permissionElements = document.querySelectorAll(`#${this.tableId} [data-tipo-id]`);
            
            if (permissionElements.length === 0) {
                console.log('❌ No se encontraron elementos de permisos en la tabla');
                return;
            }
            
            // Obtener datos
            const db = window.firebaseManager.getDB();
            const permissionsSnapshot = await db.collection('interface_permissions').get();
            
            console.log(`📊 Total permisos: ${permissionsSnapshot.size}`);
            
            // Procesar permisos por tipo de trabajador
            const permissionsByTipo = {};
            
            permissionsSnapshot.docs.forEach(doc => {
                const data = doc.data();
                
                // FORMATO 1: Permisos por interfaz específica
                if (data.tipo_trabajador_id && data.interface_name && data.activo) {
                    const tipoId = data.tipo_trabajador_id;
                    
                    if (!permissionsByTipo[tipoId]) {
                        permissionsByTipo[tipoId] = new Set();
                    }
                    
                    if (data.can_view || data.can_edit || data.can_create || data.can_delete) {
                        let permissionDetail = data.interface_name;
                        
                        const actions = [];
                        if (data.can_view) actions.push('Ver');
                        if (data.can_edit) actions.push('Editar');
                        if (data.can_create) actions.push('Crear');
                        if (data.can_delete) actions.push('Eliminar');
                        
                        if (actions.length > 0) {
                            permissionDetail += ` (${actions.join(', ')})`;
                        }
                        
                        permissionsByTipo[tipoId].add(permissionDetail);
                    }
                }
                
                // FORMATO 2: Permisos por trabajador
                else if ((data.id || data.workerId) && !data.interface_name) {
                    const tipoId = data.id || data.workerId;
                    ...
                    const interfaces = [
                        'dashboard', 'clientes', 'empleados', 'productos', 'pedidos', 
                        'compras', 'proveedores', 'estados', 'entregas', 
                        'tipos-contacto', 'tipos-pago', 'tipos-trabajador',
                        'reportes-ventas', 'reportes-productos', 'reportes-compras',
                        'balance', 'cambiar-password', 'configuracion', 'backup-sistema',
                        'manage-permissions', 'system-config', 'data-export', 'maintenance-mode'
                    ];
                    
                    interfaces.forEach(interfaceName => {
                        if (data[interfaceName] === true) {
                            permissionsByTipo[tipoId].add(interfaceName);
                        }
                    });
                }
            });
            
            console.log('📋 Permisos procesados:', permissionsByTipo);
            
            // Actualizar elementos en la tabla con layout responsivo
            for (const element of permissionElements) {
                const tipoId = element.dataset.tipoId;
                const permissionsSet = permissionsByTipo[tipoId];
                const permissions = permissionsSet ? Array.from(permissionsSet) : [];
                
                console.log(`🎯 Tipo ${tipoId}: ${permissions.length} permisos`);
                
                if (permissions.length > 0) {
                    // Función para determinar clase del badge
                    const getBadgeClass = (permission) => {
                        if (permission.includes('dashboard')) return 'badge-dashboard';
                        if (permission.includes('reportes') || permission.includes('balance')) return 'badge-reportes';
                        if (permission.includes('manage') || permission.includes('admin') || permission.includes('config')) return 'badge-admin';
                        if (permission.includes('Eliminar') || permission.includes('delete')) return 'badge-admin';
                        if (permission.includes('Editar') || permission.includes('edit')) return 'badge-config';
                        return 'badge-view';
                    };
                    
                    // Crear badges organizados
                    const permissionBadges = permissions.map(permission => {
                        const badgeClass = getBadgeClass(permission);
                        
                        // Limitar longitud para responsividad
                        let displayText = permission;
                        if (displayText.length > 15) {
                            displayText = displayText.substring(0, 12) + '...';
                        }
                        
                        return `<span class="badge ${badgeClass}" title="${permission}">${displayText}</span>`;
                    }).join('');
                    
                    // HTML con layout responsivo mejorado
                    element.innerHTML = `
                    ...
                    element.innerHTML = `
                        <div class="permissions-container">
                            <span class="badge badge-red">Sin permisos</span>
                        </div>
                        <div class="permission-summary">
                            ID: ${tipoId}
                        </div>
            ...
            console.log('✅ Permisos cargados con layout responsivo');
            ...
            permissionElements.forEach(element => {
                element.innerHTML = `
                    <div class="permissions-container">
                        <span class="badge badge-red">Error: ${error.message.substring(0, 15)}...</span>
                    </div>
                `;
            });
        }
