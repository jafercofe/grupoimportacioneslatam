class ReportesManager {
    ...
    async aplicarFiltrosVentas() {
        try {
            ...
            const estadoServicio = document.getElementById('ventas-estado-servicio')?.value; // NUEVO ID
            ...
            this.ventasFiltradas = this.ventasData.filter(venta => {
                ...
                // NUEVO: Filtro por estado de servicio usando PED_SERVICIO directamente
    ...
    async configurarFiltrosVentas() {
        ...
        // NUEVO: Configurar filtro de estados de servicio usando PED_SERVICIO
        const ventasEstadoServicio = document.getElementById('ventas-estado-servicio');
        if (ventasEstadoServicio) {
            ventasEstadoServicio.innerHTML = '<option value="">Todos los estados de servicio</option>';
            
            // Extraer estados de servicio únicos directamente de PED_SERVICIO
            const estadosServicioUnicos = [...new Set(this.ventasData.map(v => v.PED_SERVICIO))].filter(Boolean);
            
            console.log(`📊 Estados de servicio únicos encontrados:`, estadosServicioUnicos);
            
            estadosServicioUnicos.forEach(estadoServicio => {
                const option = document.createElement('option');
                option.value = estadoServicio;
                option.textContent = estadoServicio;
                ventasEstadoServicio.appendChild(option);
            });
            
            console.log(`✅ ${estadosServicioUnicos.length} estados de servicio agregados al filtro`);
        }
    ...
    actualizarEstadisticasVentas() {
        ...
        // ACTUALIZAR lógica de pendientes para usar PED_SERVICIO directamente
        const pendientes = this.ventasFiltradas.filter(venta => {
            // Considerar como pendientes los que NO están "Terminado"
