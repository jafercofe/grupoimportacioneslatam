class ReportesManager {
    ...
    async aplicarFiltrosCompras() {
        try {
            ...
            const estadoServicio = document.getElementById('compras-estado-servicio')?.value; // FIX 2: Nuevo filtro
            const numeroCompra = document.getElementById('compras-numero-compra')?.value; // FIX 3: Nuevo filtro
            ...
            console.log('🎯 Filtros aplicados:', {
                fechaDesde, fechaHasta, proveedor, estadoServicio, numeroCompra, tipoPago, montoDesde, montoHasta
            });
            ...
            this.comprasFiltradas = this.comprasData.filter(compra => {
                ...
                if (fechaDesde && compra.COM_FECHA_COMPRA) {
                    const fechaCompra = new Date(compra.COM_FECHA_COMPRA);
                    if (fechaCompra < new Date(fechaDesde)) incluir = false;
                }

                if (fechaHasta && compra.COM_FECHA_COMPRA) {
                    const fechaCompra = new Date(compra.COM_FECHA_COMPRA);
                    if (fechaCompra > new Date(fechaHasta)) incluir = false;
                }

                // Filtro por proveedor
                if (proveedor && compra.COM_PROVEEDOR !== proveedor) incluir = false;

                // FIX 2: Filtro por estado de servicio
                if (estadoServicio) {
                    const estadoCompra = compra.COM_ESTADO_SERVICIO || 'Pendiente';
                    if (estadoServicio === 'Terminado') {
                        const estadoLower = estadoCompra.toLowerCase();
                        if (!estadoLower.includes('terminado') && !estadoLower.includes('completo') && !estadoLower.includes('finalizado')) {
                            incluir = false;
                        }
                    } else if (estadoServicio === 'Pendiente') {
                        const estadoLower = estadoCompra.toLowerCase();
                        if (estadoLower.includes('terminado') || estadoLower.includes('completo') || estadoLower.includes('finalizado')) {
                            incluir = false;
                        }
                ...
                // FIX 3: Filtro por número de compra
                if (numeroCompra && numeroCompra.trim() !== '') {
                    const numeroCompraBusqueda = numeroCompra.trim().toLowerCase();
                    const numeroCompraReal = (compra.COM_NUM || '').toLowerCase();
                    if (!numeroCompraReal.includes(numeroCompraBusqueda)) {
                        incluir = false;
                    }
                }
                ...
                if (tipoPago) {
                    const tieneTipoPago1 = compra.COM_TIPO_PAGO === tipoPago;
                    const tieneTipoPago2 = compra.COM_TIPO_PAGO2 === tipoPago;
                    if (!tieneTipoPago1 && !tieneTipoPago2) {
                        incluir = false;
                    }
                }
            ...
            window.notificationService.error('Error aplicando filtros');
        }
    }
    ...
    limpiarFiltrosCompras() {
        const elementos = [
            ...
            'compras-fecha-hasta', 
            'compras-proveedor',
            'compras-estado-servicio',  // FIX 2: Nuevo ID
            'compras-numero-compra',    // FIX 3: Nuevo ID
            'compras-tipo-pago',
            'compras-monto-desde', 
            'compras-monto-hasta'
        ];
        ...
        // Aplicar filtros vacíos para mostrar todo
        this.aplicarFiltrosCompras();
        
        console.log('✅ Filtros de compras limpiados');
    }
