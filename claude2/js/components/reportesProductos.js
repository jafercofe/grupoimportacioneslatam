// ========================================
// SOLUCIÓN COMPLETA: reportesProductos.js
// Basado en tu código de GitHub + función actualizarGraficoProductos
// ========================================
...
class ReportesProductos {
    ...
    actualizarGraficoProductos() {
        ...
        if (!fechaDesde && !fechaHasta) {
            this.aplicarFiltrosPorAgrupacion(agrupacion);
        } else {
            // Si hay fechas específicas, solo regenerar gráficos
            this.actualizarGraficos();
        }
        
        // Mensaje informativo
        if (window.notificationService) {
            window.notificationService.info(`Vista actualizada: ${agrupacion}`);
        }
    }

    // NUEVA FUNCIÓN: Aplicar filtros automáticos según agrupación
    aplicarFiltrosPorAgrupacion(agrupacion) {
        ...
        switch (agrupacion) {
            case 'año':
                ...
                fechaHasta = `${ahora.getFullYear()}-12-31`;
                console.log('📅 Filtro automático: Año completo', ahora.getFullYear());
                break;
                
            case 'mes':
                const mes = String(ahora.getMonth() + 1).padStart(2, '0');
                const ultimoDiaMes = new Date(ahora.getFullYear(), ahora.getMonth() + 1, 0).getDate();
                fechaDesde = `${ahora.getFullYear()}-${mes}-01`;
                fechaHasta = `${ahora.getFullYear()}-${mes}-${ultimoDiaMes}`;
                console.log('📆 Filtro automático: Mes completo', `${ahora.getFullYear()}-${mes}`);
                break;
            ...
            case 'semana':
                ...
                console.log('📊 Filtro automático: Semana actual');
                break;
            ...
            case 'dia':
                ...
                fechaDesde = hoy;
                fechaHasta = hoy;
                console.log('🗓️ Filtro automático: Día actual');
                break;
        ...
        this.aplicarFiltrosConFechas(fechaDesde, fechaHasta);
    }

    // FUNCIÓN AUXILIAR: Aplicar filtros con fechas específicas
    async aplicarFiltrosConFechas(fechaDesde, fechaHasta) {
        try {
            ...
            console.log('Aplicando filtros con agrupación:', { fechaDesde, fechaHasta, agrupacion, productoFiltro, tipoAnalisis, cantidadMinima });
            ...
            console.log('✅ Filtros con agrupación aplicados exitosamente');

        } catch (error) {
            console.error('❌ Error aplicando filtros con agrupación:', error);
            window.notificationService.error('Error al aplicar filtros por agrupación');
        }
    }
    ...
    async aplicarFiltros() {
        try {
            ...
            const tipoAnalisis = document.getElementById('productos-tipo-analisis')?.value || 'completo';
            const cantidadMinima = parseInt(document.getElementById('productos-cantidad-minima')?.value) || 0;
            const agrupacion = document.getElementById('productos-agrupacion')?.value || 'mes'; // AGREGADO

            console.log('Filtros aplicados:', { fechaDesde, fechaHasta, productoFiltro, tipoAnalisis, cantidadMinima, agrupacion });
    ...
    actualizarTabla() {
        const tbody = document.getElementById('productos-table-reports'); // Usar ID correcto de tu HTML
        if (!tbody) return;
    ...
    limpiarFiltros() {
        ...
        const agrupacionSelect = document.getElementById('productos-agrupacion');
        if (agrupacionSelect) {
            agrupacionSelect.value = 'mes';
        }
...
/*
1. REEMPLAZA tu archivo reportesProductos.js con este código completo

2. AGREGA al final de tu index.html (antes de </body>):
   <script src="js/components/reportesProductos.js"></script>

3. AGREGA esta pestaña en tu navegación (dentro de .nav-buttons):
...
4. AGREGA el HTML que me enviaste (el div completo de reportes-productos) dentro de .content-area

5. MODIFICAR tu main.js para incluir manejo de la nueva pestaña:
   En la función loadTabData(), después de dashboard:
   else if (tabId === 'reportes-productos') {
       console.log('📈 Initializing Reportes Productos...');
       if (window.reportesProductos) {
           await window.reportesProductos.initialize();
           setTimeout(() => {
               window.reportesProductos.aplicarFiltros();
           }, 500);
       }
   }

FUNCIONALIDAD COMPLETADA:
✅ Tu campo "Agrupar por" ya funcionará correctamente
✅ Cuando cambies la agrupación y no tengas fechas, aplicará filtros automáticos
✅ Los gráficos se actualizarán correctamente
✅ Usa todos los IDs correctos de tu HTML existente
*/