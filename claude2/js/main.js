class CRMApplication {
    ...
    setupGlobalEventListeners() {
        ...
        window.deleteItem = async (type, id) => {
            ...
            // Mensaje específico para pedidos con eliminación en cascada
            const isOrder = type === 'pedido';
            const message = isOrder 
                ? '¿Estás seguro de que deseas eliminar este pedido? Se eliminarán también todos los detalles asociados. Esta acción no se puede deshacer.'
            ...
            try {
                ...
                const loadingToast = window.notificationService.info(
                    isOrder ? 'Eliminando pedido y detalles...' : 'Eliminando registro...'
                );
                ...
                const successMessage = isOrder 
                    ? 'Pedido y todos sus detalles eliminados exitosamente'
                ...
                // NUEVO: Refresh automático de página para pedidos
                if (isOrder) {
                ...
                const errorMessage = isOrder 
                    ? `Error al eliminar pedido y detalles: ${error.message}`
