...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    try {
        ...
        // 2. SEGUNDO: Inicializar servicios que dependen de Firebase
        console.log('Paso 2: Inicializando servicios...');
        
        // 3. TERCERO: Inicializar la aplicación principal
        console.log('Paso 3: Inicializando aplicación principal...');
        if (window.crmApp && window.crmApp.initialize) {
            await window.crmApp.initialize();
        }
        
        // 4. CUARTO: Inicializar sistema de permisos (después de todo lo demás)
        console.log('Paso 4: Inicializando sistema de permisos...');
        setTimeout(async () => {
            try {
                if (window.permissionManager) {
                    await window.permissionManager.initialize();
                    ...
                    console.warn('Permission manager no disponible');
                }
            } catch (permError) {
                console.error('Error inicializando permisos:', permError);
                // Continuar sin permisos en caso de error
            }
        }, 2000); // Esperar 2 segundos después de que todo lo demás esté listo
