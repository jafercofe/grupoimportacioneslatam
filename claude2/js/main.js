class CRMApplication {
    ...
    async initialize() {
        try {
            ...
            // 🔥 NUEVO: Initialize Authentication
            const authResult = await window.authService.initialize();
        
            if (!authResult) {
                console.log('❌ Usuario no autenticado, redirigiendo...');
                return; // authService.redirectToLogin() se ejecutará automáticamente
            }
            
            // 🔥 NUEVO: Proteger DataService con middleware
            window.authMiddleware.wrapDataService();
            ...
            // 🔥 NUEVO: Mostrar información del usuario en la UI
    ...
    displayUserInfo() {
        ...
        if (user) {
            // Agregar barra de usuario en la interfaz
            const userBar = document.createElement('div');
            userBar.className = 'user-bar';
            userBar.innerHTML = `
                <div class="user-info">
                    <span class="user-name">${user.nombre || user.email}</span>
                    <span class="user-role badge badge-blue">${user.rol}</span>
                    <button onclick="window.authService.logout()" class="btn btn-danger btn-sm">
    ...
    setupGlobalEventListeners() {
        ...
        const module = window.authMiddleware.getModuleFromCollection(
            window.dataService.getCollectionName(type)
        );
        
        if (!window.authService.hasPermission(module, 'delete')) {
            window.notificationService.error('Sin permisos para eliminar registros');
            return;
        }
        ...
        try {
            // El middleware interceptará y verificará permisos automáticamente
            const collectionName = window.dataService.getCollectionName(type);
            await window.dataService.delete(collectionName, id);
            
            window.notificationService.success('Registro eliminado exitosamente');
