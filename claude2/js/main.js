class CRMApplication {
    ...
    async loadTabData(tabId) {
        if (tabId === 'dashboard') {
            await this.loadDashboard();
        } else if (tabId === 'permisos') {
            console.log('Pestaña de permisos - manejada por PermissionManager');
            return;
    ...
    async refreshCurrentTable() {
        try {
            ...
            const tabToPaginationManager = {
                ...
                'tipos-pago': 'tipos-pago',
                'tipos-trabajador': 'tipos-trabajador'  // ✅ ESPECÍFICO PARA ESTE PROBLEMA
            };
            
            const managerKey = tabToPaginationManager[currentTab];
            console.log('🔍 Manager key for tab:', managerKey);
            
            if (!managerKey) {
                console.error('❌ No manager key found for tab:', currentTab);
                console.log('Available tabs:', Object.keys(tabToPaginationManager));
            ...
            if (!this.paginationManagers || !this.paginationManagers[managerKey]) {
                console.error('❌ No pagination manager found for:', managerKey);
                console.log('Available managers:', Object.keys(this.paginationManagers || {}));
                // Fallback al método anterior
                await this.loadTabData(currentTab);
                return;
            }
            
            const manager = this.paginationManagers[managerKey];
            console.log('✅ Found manager:', managerKey);
            
            // ✅ INVALIDAR CACHE ESPECÍFICO
            const typeToCollection = {
                'clientes': 'clientes',
                'empleados': 'empleados',
                'productos': 'tb_productos', 
                'pedidos': 'tb_pedido',
                'compras': 'TB_COMPRAS',
                'proveedores': 'TB_PROVEEDORES',
                'estados': 'TB_ESTADO',
                'entregas': 'entregas',
                'tipos-contacto': 'tipos_contacto',
                'tipos-pago': 'tipos_pago',
                'tipos-trabajador': 'tipos_trabajador'  // ✅ ESPECÍFICO
            };
            
            const collectionName = typeToCollection[currentTab];
            if (collectionName) {
                console.log('🧹 Invalidating cache for collection:', collectionName);
                window.smartCache.invalidate(`collection_${collectionName}`);
            }
    ...
    // En main.js, dentro de setupSearchFunctionality()
    setupSearchFunctionality() {
        ...
        this.showTab = async function(tabId) {
            ...
            if (excludedTabs.includes(tabId)) {
                console.log(`Configuración de búsqueda saltada para: ${tabId}`);
                return;
            }
            ...
            setTimeout(() => {
                if (window.SearchManager) {
                    ...
                    // Configuración avanzada para compras si es necesario
                    else if (tabId === 'compras') {
                        window.SearchManager.setupAdvancedCompraSearch();
                    }
                } else {
...
async function initializeAppWithPermissions() {
    try {
        console.log('Inicializando aplicación con sistema de permisos...');
        ...
        // 4. Aplicar permisos a la interfaz después de que todo esté cargado
        setTimeout(async () => {
            if (window.permissionManager) {
        ...
        console.log('Aplicación inicializada correctamente con permisos');
...
document.addEventListener('DOMContentLoaded', async () => {
    console.log('DOM cargado, iniciando aplicación con loading de permisos...');
    
    // MOSTRAR LOADING INMEDIATAMENTE
    window.permissionsLoadingManager.show();
    
    try {
        // 1. Crear instancia de CRM App
        window.permissionsLoadingManager.updateStep(0, 'Inicializando aplicación...', 'Configurando sistemas principales');
        console.log('Paso 1: Creando instancia de CRM App...');
        window.crmApp = new CRMApplication();
        console.log('✅ CRM App instancia creada');
        
        // 2. Inicializar CRM App
        window.permissionsLoadingManager.updateStep(1, 'Cargando datos...', 'Conectando con base de datos');
        console.log('Paso 2: Inicializando CRM App...');
        await window.crmApp.initialize();
        console.log('✅ CRM App inicializado correctamente');
        
        // 3. Inicializar sistema de permisos con progreso detallado
        window.permissionsLoadingManager.updateStep(2, 'Verificando permisos...', 'Obteniendo configuración de usuario');
        console.log('Paso 3: Inicializando sistema de permisos...');
        ...
        // 4. Aplicar permisos
        window.permissionsLoadingManager.updateStep(3, 'Aplicando configuración...', 'Configurando interfaz de usuario');
        
        // Pequeña pausa para que el usuario vea el progreso
        await new Promise(resolve => setTimeout(resolve, 500));
        
        window.permissionsLoadingManager.updateStep(4, 'Finalizando...', 'Preparando interfaz');
        
        // Pausa final antes de ocultar loading
        ...
        window.permissionsLoadingManager.updateStep(4, 'Error en inicialización', 'Activando modo de emergencia...');
        await new Promise(resolve => setTimeout(resolve, 1000));
        
        // Crear CRM de emergencia si es necesario
        if (!window.crmApp) {
            console.log('🔧 Creando CRM App de emergencia...');
            window.crmApp = new CRMApplication();
            // ... código de emergencia ...
        }
        
        alert('Error inicializando la aplicación. Algunas funciones pueden estar limitadas.');
    } finally {
        // OCULTAR LOADING AL FINALIZAR (exitoso o con error)
        setTimeout(() => {
            window.permissionsLoadingManager.hide();
        }, 500);
    }
});
...
async function initializePermissionSystem() {
    try {
        ...
        // Esperar a que el permission manager esté listo
        const maxWait = 10000; // 10 segundos máximo
        const interval = 500; // Check cada 500ms
        let elapsed = 0;
        
        while (!window.permissionManager?.isInitialized && elapsed < maxWait) {
            await new Promise(resolve => setTimeout(resolve, interval));
            elapsed += interval;
        }
        
        if (!window.permissionManager?.isInitialized) {
            console.warn('Permission manager no se inicializó a tiempo');
            return;
        }
        
        // Aplicar permisos a la interfaz
        await window.permissionManager.applyPermissionsToInterface();
        console.log('Sistema de permisos aplicado correctamente');
...
window.checkInitializationStatus = function() {
    ...
    console.log('CRM App:', !!window.crmApp);
    console.log('CRM App initialized:', window.crmApp?.initialized || false);
    console.log('Firebase Manager:', window.firebaseManager?.isInitialized() || false);
    console.log('Data Service:', !!window.dataService);
    console.log('Permission Manager:', window.permissionManager?.isInitialized || false);
    console.log('Firebase DB:', !!window.firebaseManager?.getDB());
    console.log('================================');
};

window.testNavigation = function(tabId = 'clientes') {
    console.log('Probando navegación...');
    if (window.crmApp && window.crmApp.showTab) {
        console.log(`Navegando a: ${tabId}`);
        window.crmApp.showTab(tabId);
    } else {
        console.error('crmApp.showTab no disponible');
    }
};

// Función para forzar aplicación de permisos (para debug)
window.forceApplyPermissions = async function() {
    try {
        if (!window.permissionManager) {
        ...
        console.log('Forzando aplicación de permisos...');
        await window.permissionManager.applyPermissionsToInterface();
        console.log('Permisos aplicados manualmente');
        
    } catch (error) {
        console.error('Error aplicando permisos manualmente:', error);
    }
};

window.verifyTiposTrabajadorState = async () => {
    console.log('=== VERIFICACIÓN COMPLETA ===');
    
    // 1. Verificar datos en Firebase directamente
    try {
        const db = window.firebaseManager.getDB();
        const snapshot = await db.collection('tipos_trabajador').get();
        console.log('📊 Firebase count:', snapshot.size);
    ...
    // 2. Verificar cache
    const cacheData = window.smartCache.get('collection_tipos_trabajador');
    console.log('💾 Cache count:', cacheData?.length || 0);
    console.log('💾 Cache data:', cacheData);
    
    // 3. Verificar tabla DOM
    const tableBody = document.getElementById('tipos-trabajador-table');
    console.log('🖥️ DOM rows:', tableBody?.children?.length || 0);
    
    // 4. Verificar manager
    const manager = window.crmApp?.paginationManagers['tipos-trabajador'];
    console.log('⚙️ Manager page:', manager?.currentPage);
    console.log('⚙️ Manager cache size:', manager?.pageCache?.size);
};

// ✅ PATCH DIRECTO PARA EL MÉTODO refreshCurrentTab EXISTENTE
if (window.crmApp) {
    const originalRefreshCurrentTab = window.crmApp.refreshCurrentTab;
    window.crmApp.refreshCurrentTab = async function() {
        console.log('🔄 Using enhanced refresh method...');
        try {
            await this.refreshCurrentTable();
        } catch (error) {
            console.error('Enhanced method failed, using original:', error);
            return originalRefreshCurrentTab.call(this);
        }
    };
    
    console.log('✅ Enhanced refresh method patched');
}