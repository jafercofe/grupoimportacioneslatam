class DataService {
    ...
    async update(collectionName, id, data) {
        try {
            // Verificar si es una compra y si cambió el estado a "Terminado"
            if (collectionName === 'TB_COMPRAS' && data.COM_ESTADO_SERVICIO === 'Terminado') {
                // Obtener el estado anterior
                const currentData = await this.getById(collectionName, id);
                const wasAlreadyFinished = currentData && currentData.COM_ESTADO_SERVICIO === 'Terminado';
                const alreadyProcessed = await this.checkIfCompraAlreadyProcessed(id);
                
                // Solo procesar si es la primera vez que se marca como terminado y no se ha procesado antes
                if (!wasAlreadyFinished && !alreadyProcessed) {
                    console.log('Compra cambiando a estado Terminado - procesando inventario...');
                    ...
                    // Procesar inventario
                    const inventoryResult = await this.processCompraInventoryUpdate(id, data);
                    ...
                    console.log('Compra ya estaba terminada o ya fue procesada - omitiendo actualización de inventario');
                }
            }
            
            // Proceso normal de actualización para otros casos
            if (collectionName === 'TB_ESTADO') {
                const processedData = {
                    EST_DESCRIP: data.descripcion || data.EST_DESCRIP || '',
                    descripcion: data.descripcion || data.EST_DESCRIP || ''
    ...
    async getInventorySummary() {
        try {
            ...
            products.forEach(product => {
                ...
                const price = parseFloat(product.PRO_PRECIO) || 0;
                
                summary.totalStockValue += (stock * price);
                ...
                } else if (stock <= 10) { // Considerar stock bajo si es 10 o menos
    ...
    // Función para revertir procesamiento de inventario (en caso de error)
    async revertCompraInventoryUpdate(compraId) {
        try {
            console.log('Revirtiendo actualización de inventario para compra:', compraId);
            
            const compra = await this.getById('TB_COMPRAS', compraId);
            if (!compra || !compra.COM_NUM) {
                throw new Error('Compra no encontrada');
            }
            
            const detallesCompra = await this.getCompraDetalles(compra.COM_NUM);
            ...
            for (const detalle of detallesCompra) {
                ...
                try {
                    const product = await this.getById('tb_productos', productoId);
                    if (product) {
                        ...
                        await this.update('tb_productos', productoId, {
                            PRO_CANTIDAD: newStock
                        });
                        
                        console.log(`Stock revertido para ${productoId}: ${currentStock} -> ${newStock}`);
                    }
                } catch (error) {
            ...
            // Marcar compra como no procesada
            ...
            window.notificationService.success('Actualización de inventario revertida');
     ...
     async create(collectionName, data) {
        try {
            ...
            if (!data || Object.keys(data).length === 0) {
                throw new Error('No se pueden crear documentos con datos vacíos');
            }
            ...
            console.log(`Sanitized data for ${collectionName}:`, sanitizedData);
            
            const db = window.firebaseManager.getDB();
            
            // Verificar que la colección y la base de datos estén disponibles
            if (!db) {
    ...
    async update(collectionName, id, data) {
        try {
            ...
            if (!data || Object.keys(data).length === 0) {
                throw new Error('No se pueden actualizar documentos con datos vacíos');
            }
...
async processCompraInventoryUpdate(compraId, compraData) {
    try {
        console.log('Procesando actualización de inventario para compra:', compraId);
        
        // Verificar que el estado sea "Terminado"
        if (compraData.COM_ESTADO_SERVICIO !== 'Terminado') {
            console.log('Compra no está terminada, no se actualiza inventario');
        ...
        if (detallesCompra.length === 0) {
            console.log('No hay detalles para procesar en la compra');
            return { success: true, itemsProcessed: 0 };
        }
        
        console.log(`Procesando ${detallesCompra.length} items de la compra`);
        ...
        for (const detalle of detallesCompra) {
            ...
            if (cantidad <= 0) {
                console.warn(`Cantidad inválida para producto ${productoId}:`, cantidad);
                continue;
            }
            ...
            try {
                ...
                if (existingProduct) {
                    ...
                    // Actualizar precio si el precio de compra es más reciente/diferente
                    const updateData = {
                        PRO_CANTIDAD: newStock
                    };
                    
                    // Opcional: actualizar precio si es diferente
                    if (Math.abs(parseFloat(existingProduct.PRO_PRECIO) - precioUnitario) > 0.01) {
                        updateData.PRO_PRECIO = precioUnitario;
                        console.log(`Actualizando precio de ${productoId}: ${existingProduct.PRO_PRECIO} -> ${precioUnitario}`);
                    }
                    
                    await this.update('tb_productos', productoId, updateData);
                    ...
                    inventoryUpdates.push({
                        ...
                        newPrice: updateData.PRO_PRECIO || existingProduct.PRO_PRECIO
                    });
                    
                    console.log(`Inventario actualizado - ${existingProduct.PRO_NOMBRE}: ${currentStock} -> ${newStock}`);
                    
                } else {
                    // Producto no existe - crear nuevo
                    const newProductData = {
                        PRO_ID: productoId,
                        PRO_NOMBRE: `Producto ${productoId}`, // Nombre temporal
                        PRO_CANTIDAD: cantidad,
                        PRO_PRECIO: precioUnitario,
                        PRO_OBS: `Creado automáticamente desde compra ${compraNum}`
                    };
                    
                    await this.create('tb_productos', newProductData);
                    ...
                    console.log(`Nuevo producto creado - ${productoId}: ${cantidad} unidades a ${precioUnitario}`);
                }
        ...
        // Invalidar cache de productos para refrescar la tabla
        window.smartCache.invalidate('collection_tb_productos');
        ...
        console.log('Actualización de inventario completada:', result);
...
// Función para mostrar notificación detallada de la actualización de inventario
showInventoryUpdateNotification(result) {
    if (!result.success) return;
    
    const { inventoryUpdates, newProducts, itemsProcessed } = result;
    ...
    if (newProducts.length > 0) {
        message += `\n🆕 ${newProducts.length} productos nuevos agregados`;
    }
    
    // Mostrar notificación extendida
    window.notificationService.success(message, 5000); // 5 segundos
    
    // Log detallado para debugging
    console.group('📊 Resumen de Actualización de Inventario');
    console.log('Compra:', result.compraNum);
...
// Función para verificar si una compra ya procesó su inventario
async checkIfCompraAlreadyProcessed(compraId) {
    try {
        const compra = await this.getById('TB_COMPRAS', compraId);
        
        // Podemos agregar un campo COM_INVENTARIO_PROCESADO para rastrear esto
        return compra && compra.COM_INVENTARIO_PROCESADO === true;
    } catch (error) {
