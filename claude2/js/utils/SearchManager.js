...
class SearchManager {
    ...
    constructor() {
    // Módulos que no tienen funcionalidad de búsqueda
    ...
    initialize() {
        // 1. Set up search for each module/table
        const allTabs = document.querySelectorAll('[data-tab]');
        
        allTabs.forEach(tab => {
            const tabId = tab.getAttribute('data-tab');
            
            // Skip modules that don't need search
            if (this.excludedModules.includes(tabId)) {
                return;
            }
            
            // Configure search for this module
            const searchInputId = `${tabId}-search`;
            const tableId = `${tabId}-table`;
            this.setupTableSearch(searchInputId, tableId);
        });
        
        // 2. Set up any special search configurations
        this.setupAdvancedPedidoSearch();
        
        // 3. Configure observers for dynamic content
        this.setupTabObserver();
    }
    ...
    static setupTableSearch(searchInputId, tableId) {
        ...
        const moduleId = searchInputId.replace('-search', '');
        
        try{
            const excludedModules = ['reportes-ventas', 'reportes-compras', 'reportes-productos', 'balance', 'cambiar-password', 'permisos'];
            if (excludedModules.includes(moduleId)) {
                console.log(`Búsqueda omitida para módulo: ${moduleId}`);
            return;
        }
        }catch(error){
            if (error.message.includes('reportes-')) {
                console.log('Búsqueda omitida para módulo de reportes');
                return;
            }
    ...
    static reinitializeTableSearch(tableId) {
        ...
        // CORREGIR: usar tableId en lugar de tabId
        if (excludedModules.includes(tableId)) {
            console.log(`Reinicialización omitida para módulo excluido: ${tableId}`);
        ...
        const observer = new MutationObserver((mutations) => {
            mutations.forEach((mutation) => {
                if (mutation.type === 'attributes' && 
                    ...
                    if (typeof tabId !== 'undefined' && excludedModules.includes(tabId)) {
                        console.log(`Reinicialización omitida para módulo excluido: ${tabId}`);
                        return;
                    }

                    if (excludedModules.includes(tabId)) {
                        return; // No reinicializar búsqueda para módulos excluidos
                    }
